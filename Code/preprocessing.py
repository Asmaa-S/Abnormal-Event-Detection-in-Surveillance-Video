  ...
  '''
    ...
    '''
        for i in tqdm(range(num_videos)):
            ...
            with h5py.File(os.path.join(video_root_path,
                                        '{0}/{1}_h5_t{2}/{0}_{3:02d}.h5'.format(dataset, train_or_test, time_length,
                                                                                i + 1)), 'w') as f:
  ...
  '''
    ...
    for n, f in enumerate(tqdm(filelist)):
        ...
        if n == 0:
            ...
            create_dataset = output_file.create_dataset('data', (total_rows, t, 227, 227, 1),
                                                        maxshape=(None, t, 227, 227, 1))
...
def preprocess_data(logger, dataset, t, video_root_path):
    ...
    try:
        ...
        for frame_folder in os.listdir(training_frame_path):
            training_frame_npy = os.path.join(video_root_path, dataset, "training_numpy",
                                              'training_frames_{}.npy'.format(frame_folder[-3:]))
            assert (os.path.isfile(training_frame_npy))
        for frame_folder in os.listdir(testing_frame_path):
            testing_frame_npy = os.path.join(video_root_path, dataset, "testing_numpy",
                                             'testing_frames_{}.npy'.format(frame_folder[-3:]))
