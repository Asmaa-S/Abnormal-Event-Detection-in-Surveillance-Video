...
def get_gt_vid(video_root_path, dataset, vid_idx, pred_vid):
    ...
    gt_vid_raw = np.loadtxt(
        '{0}/{1}/gt_files/gt_{2}_vid{3:02d}.txt'.format(video_root_path, dataset, gt_data, vid_idx + 1))
...
def t_predict_video(model, X_test, t=4):
    ...
    sa = np.array(
        [np.linalg.norm(np.subtract(np.squeeze(sequences[i]), np.squeeze(reconstructed_sequences[i]))) for i in
         range(0, sz)])
...
def test(logger, dataset, t, job_uuid, epoch, val_loss, video_root_path, n_videos):
    ...
    for videoid in range(n_videos):
        ...
        X_test = np.load(
            os.path.join(video_root_path, '{0}/testing_numpy/testing_frames_{1:03d}.npy'.format(dataset, videoid + 1)))
